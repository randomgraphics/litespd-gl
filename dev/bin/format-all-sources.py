...
import sys, pathlib, subprocess, importlib, argparse, platform, concurrent.futures, threading, black, os
...
def format_one_kt_file(x: pathlib.Path, root_dir: pathlib.Path, lock, dry_run=False, quiet=False):
    # Check if environment JAVA_HOME is defined
    if "JAVA_HOME" not in os.environ:
        with lock:
            sys.stderr.write("JAVA_HOME is not defined. This is required to run ktfmt.\n")
        return False
    java = pathlib.Path(os.environ["JAVA_HOME"]) / "bin" / "java"

    ktfmt = root_dir / "dev/bin/ktfmt-0.53-jar-with-dependencies.jar"
    if not ktfmt.exists():
        with lock:
            sys.stderr.write(f"ktfmt jar not found: {ktfmt}\n")
        return False
    ...
    # the last parameter means to read from stdin
    cmdline = [f"{java}", "-jar", str(ktfmt.absolute()), "--kotlinlang-style", f"--stdin-name={x.absolute()}", "-"]
    ...
    try:
        formatted = subprocess.check_output(cmdline, input=code.encode("utf-8")).decode("utf-8")
    except subprocess.CalledProcessError as e:
        with lock:
            sys.stderr.write(f"ktfmt failed: {e}\n")
        return False
...
def format_one_file(x, lock, clang_format, root_dir, dry_run=False, quiet=False):
    ...
    if pathlib.Path(x).suffix == ".kt":
        return format_one_kt_file(root_dir / x, root_dir, lock, dry_run, quiet)
    else:
        # run clang-format
